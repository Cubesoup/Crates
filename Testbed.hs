...
main = do  config <- standardIOConfig
           vty <- mkVty config
...
main' vty  = do
           ...
           case ev of
                ...
                x  -> do update vty (picForImage $ img $ (show x))
                         ignore <- nextEvent vty
                         update vty $ picForImage $ img "waiting"
                         main' vty
...
splashEventMap vty = toMap [ (ctrl (chEvent 'q') , Terminate)
                           , (ctrl (chEvent 'o') , Transition { before = (\_ -> importSetup)
                                                              , after  = (\a b -> return ())
